...
def run_pipeline(year: int = 2024) -> None:
    ...
    # Bronze: ingest raw data from each source
    logger.info("Step 1/5: Ingesting Numbeo rankings")
    numbeo_raw = fetch_numbeo_rankings(year=year)
    save_bronze(numbeo_raw, "numbeo_rankings")

    logger.info("Step 2/5: Ingesting Stack Overflow survey")
    so_raw = fetch_stackoverflow_salaries()
    save_bronze(so_raw, "stackoverflow_survey")

    logger.info("Step 3/5: Ingesting World Bank country metadata")
